...
public class PersonalFinances {
        ...
        try {
            ...
            while (inFile.hasNext()) {
                ...
                if (isbn.length() == 13 && numberOnShelf >= 0) {
                   Book b = new Book (isbn, title, author, pages, price, numberOnShelf);
                    numberOfRecords++;
                }
            }
            System.out.println(numberOfRecords + " books loader.");
            inFile.close();
   ...
   public void sortByCategory(){
    ...
    public void printTable() {
        System.out.println("Check Number\tDate\tDescription\tType\tCategory\tAmount");
        for (int i = 0; i < numberOfRecords; i++) {
            System.out.println(checkNum[i] + "\t" + date[i] + "\t" + description[i] + "\t" + type[i] + "\t" + category[i] + "\t" + amount[i]);
        }
    }
    ...
    for (int i = 0; i < numberOfRecords; i++) {
        if (category[i].equals(currentCategory)) {
            System.out.println(date[i] + " " + description[i] + " " + amount[i]);
            subtotal += amount[i];
        } else {
...
/** Write a sort that sorts by check number.  Since check number is an integer, 
 * thecomparison will look like the original example code, 
 * but you still can’t use thelength of the array.  you will use the same swap method that
 * you already wrote.(b)  Write a method that will print 
 * the transactions using formatted Strings.*/
...
public void printByCheckNum(){
    sortByCheckNum();
    for (int i = 0; i < numberOfRecords; i++) {
        System.out.printf("%d %s %s %.2f %s %s\n", checkNum[i], date[i], description[i], amount[i], type[i], category[i]);
    }

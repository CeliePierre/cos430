 ...
 * type safety
 *
 * 9.2023 - Joel Kalala - I added the constructor, getter methods, toString, compareTo, and javadoc.
...
public class Song implements Comparable {
    ...
    public int compareTo(Object that) {
        if (!(that instanceof Song)) {
            throw new IllegalArgumentException("Object is not a Song instance");
        }
        Song otherSong = (Song) that;

        int artistComparison = this.artist.compareToIgnoreCase(otherSong.artist);
        if (artistComparison != 0) {
            return artistComparison;
        }
        return this.title.compareToIgnoreCase(otherSong.title);
    }

...
public class RaggedArrayList<E> implements Iterable<E> {
     ...
     *
     * @authorjoelkalala I Initialized indices for the primary array (L1Array)
    ...
    public ListLoc findFront(E item) {
        ...
        while (i1 < l1Array.length && l1Array[i1] != null) {
            ...
            if (i2 < l2Array.numUsed && comp.compare(l2Array.items[i2], item) == 0) {
                return new ListLoc(i1, i2);
            }

            // If we reached here, it means the current L2Array does not contain the target.
            // But if i2 hasn't reached the end of the L2Array, return its location as a potential insertion point.
            if (i2 < l2Array.numUsed) {
                return new ListLoc(i1, i2);
            }
            // If neither of the above conditions met, reset i2 and move to the next L1Array element
            i1++;
            i2 = 0;
        }
        // If the loops finish without returning, it means the target item isn't in the list.
        // Return the last known location for potential insertion.
    ...
    public ListLoc findEnd(E item) {
        ...
        if (size > 0) {
            ...
            while (i1 < l1Array.length && ((L2Array) l1Array[i1] == null || comp.compare(item, ((L2Array) l1Array[i1]).items[0]) >= 0)) {
                i1++;
            }
            i1--;  // Move back to the correct L1Array

            // Different loop to find the right position (i2) in the L2Array at i1 
            L2Array l2Array = (L2Array) l1Array[i1];

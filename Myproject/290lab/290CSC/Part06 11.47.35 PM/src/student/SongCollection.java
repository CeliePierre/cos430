...
public class SongCollection {
   ...
   public static void main(String[] args) {
    ...
    try {
        ...
        // Note: You'll need to implement the SongCollection class and its getAllSongs method.
        SongCollection songCollection = new SongCollection(songFile);

        // Create a SearchByLyricsWords object with the song collection
        SearchByLyricsWords search = new SearchByLyricsWords(songCollection);

        // Call statistics method to print out statistics
        search.statistics();

        // If the "-top10words" flag is set, call the top10words method
        if (top10WordsFlag) {
            search.top10words();
        }
    } catch (Exception e) {

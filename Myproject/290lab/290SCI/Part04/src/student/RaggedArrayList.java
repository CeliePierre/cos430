...
public class RaggedArrayList<E> implements Iterable<E> {
    ...
    public class ListLoc {
        ...
        public void moveToNext() {
            ...
            if (level2Index >= currentL2.numUsed) {
                // Reset level2Index.
                level2Index = 0;
     ...
     *
     * I Initialized indices for the primary array (L1Array) and secondary array
     * (L2Array). I Iterate through L1Array Cast the current element of L1Array
     * to L2Array for access through L2Array until an item that's not less than
     * the target is found If an item is found that matches the target, return
     * its location
     *
     * @author joelkalala I Initialized indices for Level 1 (L1) and Level 2
     * (L2) arrays Loop through the L1 array until the end is reached or until a
     ...
     * the next suitable position to add a new entry. this might be an unused
     * slot at the end of a level 2 array
     *
     * @author joelkalala I Initialized indices for Level 1 (L1) and Level 2
     * (L2) arrays I added Only proceed if there are items in the data structure
     * I addedLoop through L1 array to find the appropriate L2Array that could
     * contain the item. This is determined by checking if the first item in
     * each L2Array is less than or equal to the target item. Once the loop
     * ends, i1 might have moved one step too far, so adjust it if necessary I
     * Retrieved the determined L2Array from the L1 array I added a second Loop
     * through the determined L2Array to find the right position for the item or
     * until a larger item is found, or the end of the L2Array is reached.
     * Return the determined position within the L1 and L2 arrays.
     * @param item the thing we are searching for a place to put.
     * @return the location where this item should go Revised by: wbrown
     */
    ...
    public boolean contains(E item) {
        ...
        if (location.level1Index < l1Array.length) {
    ...
    public E[] toArray(E[] a) {
        // TO DO in part 5 and NOT BEFORE
        if (a.length != size) {
        throw new IllegalArgumentException("Provided array does not match the size of the list.");
    }
    ...
    private class Itr implements Iterator<E> {
        ...
        public boolean hasNext() {
            // TO DO in part 5 and NOT BEFORE
            return !(loc.level1Index == l1NumUsed - 1 && loc.level2Index == ((L2Array) l1Array[loc.level1Index]).numUsed);

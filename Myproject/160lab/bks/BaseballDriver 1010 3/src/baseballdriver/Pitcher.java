...
/**
 * A class to represent the pitchers on a team. You must
 * <ol>
 * <li> define private data members for the additional attributes that are
 * specific to pitchers, essentially a few additional statistics </li>
 ...
 * calculating aggregate statistics for the the team's pitching</li>
...
public class Pitcher extends Player {
    ...
    private final int inningsPitched;     // The number of innings pitched by the pitcher
    private final int runs;               // The number of runs given up by the pitcher
    private  double earnedRunAverage;  // The pitcher's earned run average
    private  double whip;            // The pitcher's WHIP (walks and hits per inning pitched)
...
private int getAtBats;
    // class implementation goes here
    public Pitcher(int number, String name, int position, boolean throwsRightHanded,
            int plateAppearances, int walks, int strikeouts, int hits,
            int inningsPitched, int runs) {
        super(number, name, position, throwsRightHanded, plateAppearances, walks, strikeouts, hits);
        this.inningsPitched = inningsPitched;
        this.runs = runs;
        this.name = name;
        this.number = number;
        this.earnedRunAverage = earnedRuns;
       calculateERAandWHIP();
    }

    
     // Getters
    public void calculateERAandWHIP() {
    if (inningsPitched > 0) {
        earnedRunAverage = (double) earnedRuns / inningsPitched * 9.0;
...
public int getInningsPitched() {
 ...
 public int getgetAtBats(){
    ...
    public String toString() {
       String eraStr = (earnedRunAverage == 0) ? "n/a" : String.format("%.2f", earnedRunAverage);
    String whipStr = (whip == 0) ? "n/a" : String.format("%.2f", whip);
    
    return "PA: " + plateAppearances + " BB: " + walks + " SO: " + strikeouts
            + " H: " + hits + " IP: " + inningsPitched / 3 + " " + inningsPitched % 3 + "/3 ER: "
            + runs + " ERA: " + eraStr + " WHIP: " + whipStr;

    
        
       // return "PA: " + getPlateAppearances() + " BB: " + getWalks() + " SO: " + getStrikeouts()
               // + " H: " + getHits() + " IP: " + inningsPitched/3 + " " + inningsPitched % 3 + "/3 ER: "
               // + runs + " ERA: " + String.format("%.2f", earnedRunAverage) + " WHIP: "
               // + String.format("%.2f", whip);
        
    }

    /**
     * Unit test for Pitcher. There is no need to comment this out! Do NOT

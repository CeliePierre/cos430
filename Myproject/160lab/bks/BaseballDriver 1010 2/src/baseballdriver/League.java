...
public class League {
   ...
   // private Map<String, Team> teams;
    ...
    public String calcHittingStats(String team ) {
          StringBuilder str = new StringBuilder();
    if (league.containsKey(team)) {
        List<PositionPlayer> hitters = league.get(team).getHitters();
        if (hitters == null || hitters.isEmpty()) {
            ...
            for (PositionPlayer hitter : hitters) {
                totalPa += hitter.getPlateAppearances();
                totalw += hitter.getWalks();
                totalSo += hitter.getStrikeouts();
                totalH += hitter.getHits();
                totalAb += hitter.getAtBats();
                totalRbi += hitter.getRBIs();
                totalHr += hitter.getHomeRuns();
                totalHbp += hitter.getHitByPitches();
            }
            double battingAverage = totalH / (double) totalAb;
            double onBasePercentage = (totalH + totalw + totalHbp) / (double) totalPa;
            str.append("There are ").append(hitters.size());
            str.append(" hitters on the ").append(team);
            str.append(" and their aggregated statistics are: \n");
            str.append("Batting Average: ").append(String.format("%.3f", battingAverage)).append("\n");
            str.append("On-base Percentage: ").append(String.format("%.3f", onBasePercentage)).append("\n");
            str.append("Total Plate Appearances: ").append(totalPa).append("\n");
            str.append("Total Walks: ").append(totalw).append("\n");
            str.append("Total Strikeouts: ").append(totalSo).append("\n");
            str.append("Total Hits: ").append(totalH).append("\n");
            str.append("Total At Bats: ").append(totalAb).append("\n");
            str.append("Total RBIs: ").append(totalRbi).append("\n");
            str.append("Total Home Runs: ").append(totalHr).append("\n");
            str.append("Total Hit by Pitches: ").append(totalHbp).append("\n");
        }
    } else {

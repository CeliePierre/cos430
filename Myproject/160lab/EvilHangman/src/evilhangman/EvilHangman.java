...
public class EvilHangman {
   private TreeSet<String> possibilities;
    ...
    public void  createSet(){
        ...
        if(debugging){
            System.out.println("There are " + possibilities.size() + " words in the set");
        }
    }
    ...
    if(debugging){
    System.out.println("words left in set" + possibilities.size());
}
    if(possibilities.size()==1){
        gameWord = possibilities.pollFirst();
        gameWordArray = new char [gameWord.length()];
         gameWordArray = gameWord.toCharArray(); 
        if(debugging){
            System.out.println("forced to choose a word. ir is: " + gameWord);
        }
    }

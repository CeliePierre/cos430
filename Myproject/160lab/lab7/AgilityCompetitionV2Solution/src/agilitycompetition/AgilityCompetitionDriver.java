...
/**
 * Course: CSCI 160
 * Project: AgilityCompetition (Version 2)
 * Class: Dog
 * Uses: Date class
 * Extends: nothing
 * Implements: Comparable
 * Driver for the Agility Competition V2 showing the use of inheritance and
...
public class AgilityCompetitionDriver {
    ...
    public void printPressRelease() {
        ...
        System.out.printf("It was a beautiful day here at the park in "
                + "%s %nwhere the 3rd annual city wide "
                + "agility competition \nsponsored by %s"
                + " was held. Top honors go to %s,%n"  
                + "a %.2f lb %.0f year old %s"
                + "who ran the course in %n%s.\n",
                 ac.getContestLocation(), ac.getContestSponsor(), 
                 winner.getName(), winner.getWeight(), 
                 age ,winner.getBreed().trim(), timeStr);
    }

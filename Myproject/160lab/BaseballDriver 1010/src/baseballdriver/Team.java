...
public class Team {
    ...
    private Team readTeamBlock(Scanner s) {
        String name = s.next();
        int number = s.nextInt();
        TreeMap< Integer, Player> roster = new TreeMap<>();
        while (number != -1) {
            Player p = readPlayerBlock(s, number);
            roster.put(p.getPlayerNumber(), p);
            number = s.nextInt();
        }
        return new Team(name, roster);
    }

    private static Player readPlayerBlock(Scanner s, int number) {
        Player p;
        String name = s.next();
        int position = s.nextInt();
        boolean trh = (s.next().charAt(0) == 'T');
        int pa = s.nextInt();
        int w = s.nextInt();
        int so = s.nextInt();
        int h = s.nextInt();
        ...
        int v2 = s.nextInt();
        if (position == 1) {
            p = new Pitcher(number, name, position, trh, pa, w, so, h, v1, v2);
        } else {
            int v3 = s.nextInt();
            int v4 = s.nextInt();
            p = new PositionPlayer(number, name, position, trh, pa, w, so, h, v1, v2, v3, v4);
        }
        return p;
    }
    ...
    public String noTeam(int num) {

        StringBuilder str = new StringBuilder();
        str.append("no team named ").append(teamName);
        str.append(" is in the league. ");
        return str.toString();

    }
    
    private String noneOfThose (String position ){
        return ("There appear to be no " + position + " on the " + teamName + " at this time");
    }
    ...
    public String calcPitchingStats() {
        ...
        for (Integer playerNum : players.keySet()) {
            if (players.get(playerNum) instanceof Pitcher) {
                ...
                totalH += p.getHits();
                totalIp += p.getIP() + (p.getInningsPitched() / 3.0);
        ...
        if (count > 0) {
            if (totalIp > 0) {
                double teamERA = ((double) totalER / totalIp) * 9;
                ...
                str.append("PA: ").append(totalPa).append(" BB: ").append(totalBB).append(" SO: ").append(totalSO).append(" H: ").append(totalH).append(" IP: ").append(totalIp).append(" 0/3");
                str.append(" ER: ").append(totalER).append(" ERA: ").append(String.format("%.2f", teamERA)).append(" WHIP: ").append(String.format("%.2f", teamWHIP));
            } else {
                str.append("There are no innings pitched by the pitchers on the ").append(teamName);
            }
        } else {
            str.append("There are no pitchers on the ").append(teamName).append(" at this time.");
        }

        return str.toString();
    }

    public Player lookupPlayer(int num) {
        StringBuilder str = new StringBuilder() ;
        if (players.containsKey (num)) { 
         str.append("The ").append(teamName);
        str.append(" player number ").append(num);
        str.append("is a").append(players.get(num).getPositionWord());
        str.append("named").append(players.get(num).getPlayerName());
        str.append("and has statistic: \n");
        str.append(players.get(num).toString());
        } else {
             str.append("No player with number " ).append(num);
              str.append(" is on the roster for the ").append(teamName);
        }
    ...
    public String calcHittingStats() {
        ...
        if (count > 0) {
            ...
            str.append("PA:").append(totalPa).append(" w:").append(totalw).append(" So:")
                    .append(totalSo).append(" H:").append(totalH).append(" Rbi:").append(totalRbi)
                    .append(" Hr:").append(totalHr).append(" Hbp:").append(totalHbp);
            str.append(" BA:").append(String.format("%.3f", battingAverage)).append(" OBP:").append(String.format("%.3f", onBasePercentage));

        } else {
            str.append("There are no hitters on the NoHitters at this time.");
        }
